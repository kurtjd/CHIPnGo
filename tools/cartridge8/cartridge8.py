...
def browse_roms(sender, data, path):
    ...
    dpg.add_file_dialog(tag=name, show=True, modal=True, height=300,
                        callback=open_rom, user_data=path)
...
def show_fields(rom):
    ...
    with dpg.group(horizontal=True):
        dpg.add_text('       Title: ')
        title = dpg.add_input_text(label='', default_value=rom.title,
                                   width=100, uppercase=True)

    with dpg.group(horizontal=True):
        dpg.add_text('    CPU Freq: ')
        cpu_freq = dpg.add_input_int(label='', default_value=rom.cpu_freq,
                                     width=100, min_value=0, min_clamped=True)

    with dpg.group(horizontal=True):
        dpg.add_text('  Timer Freq: ')
        timer_freq = dpg.add_input_int(label='', default_value=rom.timer_freq,
...
def edit_rom(sender, data, user_data):
    ...
    right_map = [bool(int(c))
                 for c in format(rom.right_btn_map, '016b')[::-1]]
    ...
    with dpg.window(tag='ROM Edit', width=WIDTH, height=HEIGHT,
        ...
        with dpg.group(horizontal=True, indent=(WIDTH / 2) - 50):
            dpg.add_button(label='Save', callback=save_rom,
...
def rom_select(roms):
    with dpg.window(tag='ROM Select'):
        ...
        dpg.add_text(indent=65, default_value='Modifying or adding ROM will '
                     'likely result in file loss!')
        dpg.add_text(indent=120, default_value='Only use an SD card dedicated '
                     'to CHIPnGo!')

        dpg.add_spacer(height=20)
        dpg.add_text(indent=45, default_value='Select a ROM to modify, '
        ...
        for i in range(ROWS):
            with dpg.group(horizontal=True):
                for j in range(COLS):
                    rom_num = (i * COLS) + j
                    dpg.add_button(label=roms[rom_num].title, width=100,
                                   height=100, callback=edit_rom,
...
def main():
    ...
    dpg.create_context()
    dpg.create_viewport(title='Cartridge8',
